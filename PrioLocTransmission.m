function [surplus_parks,deficit_parks,region_excess_power,region_deficit_power,loc_storage_matrix,indices_below_limit,loc_power_loss] = PrioLocTransmission(surplus_parks, ...
    ...
    for r = 1:length(regions)
        ...
        if any(parks_below_limit)
            ...
            [loc_storage_matrix,indices_below_limit,loc_power_loss] = local_storage_tol_power(loc_storage_matrix,local_storage_efficiency,loc_power_capacity,base_load_tol_diff,region_Indices,parks_below_limit,loc_power_loss,t,r);
        end
        parks_below_limit = loc_storage_matrix(region_Indices, t) < loc_storage_low;
        ...
        if tot_Regional_Surplus >= tot_Regional_Deficit
            ...
            if any(parks_below_limit)
                % If any of the parks how low storage, drop power level to store energy
                [loc_storage_matrix,region_excess_power] = local_storage_excess_low_lim(loc_storage_matrix,region_excess_power,loc_storage_low,local_storage_efficiency,region_Indices,parks_below_limit,loc_power_capacity,r,t);
            end
